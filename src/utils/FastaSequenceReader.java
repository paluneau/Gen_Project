...
public final class FastaSequenceReader {
	...
	public FastaSequenceReader(String filename, ArrayList<String> target) {
	...
	void readSequenceFromFile(String file) {
		...
		try {
			...
			} else if (line.contains(targets.get(0))) {
				desc.add(line);
				read = true;
			}
	...
	private void storeData(List<String> seq, List<String> desc) {
		...
		System.out.println("desc size:" + description.length);
		System.out.println("seq size:" + sequence.length);
